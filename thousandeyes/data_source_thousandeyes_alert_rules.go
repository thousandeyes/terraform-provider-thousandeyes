...
func dataSourceThousandeyesAlertRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	// 	if found == nil {
	// 		return fmt.Errorf("unable to locate any alert rules with the name: %s", searchName)
	// 	}
	// 	log.Printf("[INFO] ###### Found alert rule rule_id: %d - name: %s", found.RuleID, found.RuleName)
	...
	// 	log.Printf("[INFO] ###### Reading Thousandeyes alert rules by id [%d]", searchRuleID)
	// 	found, err := client.GetAlertRule(searchRuleID)
	// 	if err != nil {
	// 		return err

	// 	}
	// 	if found == nil {
	// 		return fmt.Errorf("unable to locate any alert rules with the rule id: %d", searchRuleID)
	// 	}
